...
CvRect * detectFace(IplImage * pImg)
{
	...
	pFaceRectSeq = cvHaarDetectObjects
		...
		1.1,                       // increase search scale by 10% each pass 1.1
		6,                         // require six neighbors 6
		CV_HAAR_DO_CANNY_PRUNING,  // skip regions unlikely to contain a facecvSize(minFaceSize, minFaceSiz));
		cvSize(minFaceSize, minFaceSize));
	// get the sequence of face rectangles
	 //faces =
	//cvHaarDetectObjects(image, cascade, storage, 1.2, 2, CV_HAAR_DO_CANNY_PRUNING, cvSize(0, 0));
